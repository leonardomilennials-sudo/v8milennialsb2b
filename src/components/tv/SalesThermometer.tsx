    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex items-stretch gap-3 min-h-0">
        ...
        <div className="flex flex-col justify-between py-2 text-right">
          <span className="text-[10px] font-mono text-sidebar-foreground/50">{getScaleValue(100)}</span>
          <span className="text-[10px] font-mono text-sidebar-foreground/50">{getScaleValue(75)}</span>
          <span className="text-[10px] font-mono text-sidebar-foreground/50">{getScaleValue(50)}</span>
          <span className="text-[10px] font-mono text-sidebar-foreground/50">{getScaleValue(25)}</span>
          <span className="text-[10px] font-mono text-sidebar-foreground/50">0</span>
        </div>

        {/* Thermometer */}
        <div className="relative flex flex-col items-center flex-1">
          {/* Tube */}
          <div className="relative w-8 flex-1 rounded-t-full bg-sidebar-accent/60 border border-sidebar-border overflow-hidden">
            {/* Division lines */}
            {[25, 50, 75].map((p) => (
              <div 
                key={p} 
                className="absolute left-0 right-0 h-px bg-sidebar-border" 
                style={{ bottom: `${p}%` }} 
              />
            ))}
            ...
            >
              <div className="absolute -right-1 top-1/2 -translate-y-1/2 w-0 h-0 border-t-[4px] border-b-[4px] border-l-[6px] border-transparent border-l-white/40" />
            </div>
          ...
          <motion.div 
            ...
            className={`w-14 h-14 rounded-full -mt-2 flex items-center justify-center z-10 border-2 ${
              isAhead 
        ...
        <div className="relative flex-1 max-w-[80px]">
          <motion.div
            initial={{ bottom: "0%" }}
            animate={{ bottom: `${Math.max(5, percentage - 5)}%` }}
            transition={{ duration: 1.5, ease: "easeOut" }}
            className="absolute left-0 right-0"
          >
            <div className="flex items-center gap-1">
              <div className={`w-0 h-0 border-t-[5px] border-b-[5px] border-r-[7px] border-transparent ${
                isAhead ? "border-r-emerald-500" : "border-r-amber-500"
              }`} />
              <div className={`px-2 py-1 rounded text-white font-bold text-xs whitespace-nowrap ${
